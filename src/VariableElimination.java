    ...
    public void to_join(ArrayList<Factor> lst_factors) {
        ...
        for (String curr_hidden : sorted_hiddens) {
            ...
            for (int i = 0; i < arr_factors.length; i++) {
                ...
                } else { // if the factor is in factor_ignore from last iteration, remove it
                    factor_ignore.remove(arr_factors[i]);
                }
            }
            ...
            factor_ignore = sort_factors(factor_ignore);
...
//                    factor_ignore.remove(curr_factors[i + 1]); // doesn't work
                    for (Factor factor : curr_factors) {
                        for (int index = 0; index < arr_factors.length; index++) {
                            if (index < factor_ignore.size()) {
                                if (factor_ignore.get(index).cpt.equals(curr_factors[i + 1].cpt)) {
                                    factor_ignore.remove(index);
                                    set_factor = index;
                                    break;
                                }
                            }
            ...
            for (Factor factor : curr_factors) {
                for (Factor ignore : factor_ignore) {
                    if (ignore.cpt.equals(factor.cpt)) {
                        flag_ignore = true;
                        break;
                    }
                }
...
//                              }
    ...
    public String[][] eliminate(String[][] joined_cpt, String curr_hidden) {
        ...
        // has the same values.
        // no need to check the last column - it is checked by inner loop - next_row
        for (int row = 1; row < joined_cpt.length - 1; row++) {
            row_prob = 0;
            first_values = new String[new_num_col - 1]; // without prob column
            ...
            for (int col = 0; col < joined_cpt[row].length; col++) {
                if (col != index_hidden) {
                    ...
                    } else if (col == joined_cpt[row].length - 1 && !rows_ignore[row]) {
            ...
            // flag_first means the row should be in new_cpt
            if (flag_first && first_values != null) {
                    ...
                    for (int col = 0; col < joined_cpt[next_row].length; col++) {
                        if (col != index_hidden && !rows_ignore[next_row]) {
                            if (col != joined_cpt[next_row].length - 1) {
                                Boolean not_same = !(joined_cpt[next_row][col].equals(first_values[col]));
                                if (not_same) {
                                    break;
                                } else {
                                    continue;
                ...
                new_cpt[new_cpt_row][new_num_col - 1] = String.valueOf(row_prob); // add the sum of probs
